function createcasetable(segs::Array{Float64, 2}, wobrange, inclinationrange, computefunction, pdir)
  ...
  for (i, wob) in enumerate(wobrange)
    for (j, incl) in enumerate(inclinationrange)
      ...
      casetable[ (i-1) * length(inclinationrange) + j ] = (segs, wob, incl, computefunction, pdir)
